...
bool RenderPass2::initProgram()
{
	...
	// texture bind slots
	{
		int colorTex = GetUniformLocation(m_program, "material.colorTex");
		assert(colorTex > -1);
		SetUniform(colorTex, 0);

		int normalTex = GetUniformLocation(m_program, "material.normalTex");
		assert(normalTex > -1);
		SetUniform(normalTex, 1);

		int specularTex = GetUniformLocation(m_program, "material.specularTex");
		assert(specularTex > -1);
		SetUniform(specularTex, 2);

		int glossTex = GetUniformLocation(m_program, "material.glossTex");
		assert(glossTex > -1);
		SetUniform(glossTex, 3);

		int opacityTex = GetUniformLocation(m_program, "material.opacityTex");
		assert(opacityTex > -1);
		SetUniform(opacityTex, 4);
	}
